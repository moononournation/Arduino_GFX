...
void Arduino_ESP32i80::begin(int32_t speed, int8_t dataMode)
{
  pinMode(_dc, OUTPUT);
...
void Arduino_ESP32i80::beginWrite()
{
  DC_HIGH();
  CS_LOW();
}

void Arduino_ESP32i80::endWrite()
{
  CS_HIGH();
}

void Arduino_ESP32i80::writeCommand(uint8_t c)
{
  DC_LOW();
...
void Arduino_ESP32i80::writeCommand16(uint16_t c)
{
  DC_LOW();
...
void Arduino_ESP32i80::write(uint8_t d)
{
  WRITE(d);
}

void Arduino_ESP32i80::write16(uint16_t d)
{
  WRITE16(d);
}

void Arduino_ESP32i80::writeRepeat(uint16_t p, uint32_t len)
{
  _data16.value = p;
...
void Arduino_ESP32i80::writePixels(uint16_t *data, uint32_t len)
{
  while (len--)
  {
    uint16_t d = *data++;
...
void Arduino_ESP32i80::writeC8D8(uint8_t c, uint8_t d)
{
  DC_LOW();
...
void Arduino_ESP32i80::writeC8D16(uint8_t c, uint16_t d)
{
  DC_LOW();
...
void Arduino_ESP32i80::writeC8D16D16(uint8_t c, uint16_t d1, uint16_t d2)
{
  DC_LOW();
...
void Arduino_ESP32i80::writeBytes(uint8_t *data, uint32_t len)
{
  while (len > 1)
  {
    _data16.msb = *data++;
...
void Arduino_ESP32i80::writePattern(uint8_t *data, uint8_t len, uint32_t repeat)
{
  while (repeat--)
  {
    writeBytes(data, len);
...
void Arduino_ESP32i80::writeIndexedPixels(uint8_t *data, uint16_t *idx, uint32_t len)
{
  while (len--)
  {
    WRITE16(idx[*data++]);
...
void Arduino_ESP32i80::writeIndexedPixelsDouble(uint8_t *data, uint16_t *idx, uint32_t len)
{
  while (len--)
  {
...
INLINE void Arduino_ESP32i80::WRITE(uint8_t d)
{
  *_dataPortClr = _dataClrMask;
...
INLINE void Arduino_ESP32i80::WRITE16(uint16_t d)
{
  *_dataPortClr = _dataClrMask;
  _data16.value = d;
  *_dataPortSet = _xset_mask_hi[_data16.msb] | _xset_mask_lo[_data16.lsb];
...
INLINE void Arduino_ESP32i80::DC_HIGH(void)
{
  *_dcPortSet = _dcPinMask;
}

INLINE void Arduino_ESP32i80::DC_LOW(void)
{
  *_dcPortClr = _dcPinMask;
}

INLINE void Arduino_ESP32i80::CS_HIGH(void)
{
  if (_cs != GFX_NOT_DEFINED)
  {
...
INLINE void Arduino_ESP32i80::CS_LOW(void)
{
  if (_cs != GFX_NOT_DEFINED)
  {
